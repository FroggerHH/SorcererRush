...
namespace ShadowGroveGames.LoginWithDiscord.Scripts
{
    public class LoginWithDiscordScript : MonoBehaviour
    {
        ...
        [Header("Discord Settings")] [SerializeField]
        private string _applicationId;

        [SerializeField] private OAuthPermissions _requestPermissions;
        ...
        [SerializeField] [Tooltip("Log Level for the callback server")]
        ...
        [SerializeField] private OnLoginWithDiscordEvent _onLoginWithDiscordFailure = new OnLoginWithDiscordEvent();

        [SerializeField] private OnRateLimitEvent _onRateLimitWarningEvent = new OnRateLimitEvent();

        [SerializeField] private OnRateLimitEvent _onRateLimitRechedEvent = new OnRateLimitEvent();

        private OAuthToken? _oAuthToken = null;
        private string _serverStateKey;
        public LoginWithDiscordServer _loginWithDiscordServer { get; private set; }
        ...
        private void OnEnable()
        {
            ...
            _loginWithDiscordServer = new LoginWithDiscordServer(new HashSet<int>(_callbackPorts), _logLevel,
                _serverStateKey, _applicationId);
        ...
        public void LoginWithDiscordSuccess(OAuthToken oAuthToken)
        {
            ...
            PlayerPrefs.SetString("DiscordAccount", JsonConvert.SerializeObject(oAuthToken));
            PlayerPrefs.Save();
        }
        ...
        public string GenerateDiscordOAuthUrl()
        {
            ...
            return
                $"{DISCORD_AUTH_BASE_URL}?response_type=token&client_id={_applicationId}&scope={string.Join("%20", permissions)}&redirectUrl=127.0.0.1:{_loginWithDiscordServer.ActivePort}&state={_serverStateKey}";
        }
        ...
        public List<UserGuild> GetUserGuilds()
        {
            ...
            if (!ValidateRequest()) return empty;
